...
async function quoteSafe(symbol, label) {
  try {
    const q = await cachedDurable(`q:${symbol}`, QUOTE_TTL, () => yahoo.getQuote(symbol));
...
router.get("/quote/:symbol", async (req, res) => {
  try {
    const q = await cachedDurable(`q:${req.params.symbol}`, QUOTE_TTL, () =>
...
router.get("/history/:symbol", async (req, res) => {
  ...
  try {
    const h = await cachedDurable(`h:${req.params.symbol}:${range}:${interval}`, HISTORY_TTL, () =>
