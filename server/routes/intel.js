...
router.get("/intel/sectors", async (_req, res) => {
  try {
    const rows = await Promise.all(SECTORS.map(async (s) => {
      try { const q = await cached(`q:${s.symbol}`, 15_000, () => yahoo.getQuote(s.symbol)); return { ...q, label: s.label }; }
...
router.get("/quotes", async (req, res) => {
  ...
  const quotes = await Promise.all(symbols.map(async (s) => {
    try { return await cached(`q:${s}`, 15_000, () => yahoo.getQuote(s)); } catch { return { symbol: s, error: true }; }
  }));
  res.json({ quotes });
});

/* ── Library: persistent document store (JSON file; swap for Postgres later) ── */
const STORE = path.join(__dirname, "..", "data", "library.json");
function readStore() { try { return JSON.parse(fs.readFileSync(STORE, "utf8")); } catch { return { docs: [] }; } }
function writeStore(s) { fs.mkdirSync(path.dirname(STORE), { recursive: true }); fs.writeFileSync(STORE, JSON.stringify(s, null, 1)); }

router.get("/library", (_req, res) => {
  const s = readStore();
  res.json({ docs: s.docs.map(({ payload, ...meta }) => meta).sort((a, b) => b.ts - a.ts) });
});
router.get("/library/:id", (req, res) => {
  const doc = readStore().docs.find((d) => d.id === req.params.id);
  doc ? res.json(doc) : res.status(404).json({ error: "Not found" });
});
router.post("/library", express.json({ limit: "3mb" }), (req, res) => {
  const s = readStore();
  const doc = { id: Math.random().toString(36).slice(2, 10), ts: Date.now(), title: req.body.title || "Untitled", kind: req.body.kind || "report", symbol: req.body.symbol || "", payload: req.body.payload || {} };
  s.docs.push(doc); writeStore(s);
  res.json({ id: doc.id });
});
router.delete("/library/:id", (req, res) => {
  const s = readStore();
  s.docs = s.docs.filter((d) => d.id !== req.params.id);
