...
function computeRatios(bundle, st) {
  ...
  const ratios = [
    ...
    // ── Extended valuation set (after Dividend yield, per Equity Research spec) ──
    R("Valuation", "PEG", ks.pegRatio ?? (sd.trailingPE && bundle.financialData?.earningsGrowth ? sd.trailingPE / (bundle.financialData.earningsGrowth * 100) : null), "x", false, "P/E scaled by expected earnings growth — <1 suggests growth is under-priced"),
    R("Valuation", "Earnings yield", sd.trailingPE ? 100 / sd.trailingPE : (mcap ? div(li.netIncome, mcap) * 100 : null), "pct", true, "Inverse P/E — earnings return at the current price, comparable to bond yields"),
    R("Valuation", "FCF yield", mcap && lc.fcf != null ? (lc.fcf / mcap) * 100 : null, "pct", true, "Free cash flow on market cap — the cash return the business actually produces"),
    R("Valuation", "P/S", mcap && li.revenue ? mcap / li.revenue : null, "x", false, "Price on sales — useful where earnings are depressed or in transition"),
    R("Valuation", "EV / FCF", ev !== null && lc.fcf ? ev / lc.fcf : null, "x", false, "Enterprise value on free cash flow — capital-structure-neutral cash multiple"),
  ].filter((r) => r.value !== null && Number.isFinite(r.value));
  // de-duplicate by name (extended set may overlap Yahoo-provided PEG / EV-Sales)
  const seenNames = new Set();
  const dedupedRatios = ratios.filter((r) => (seenNames.has(r.name) ? false : (seenNames.add(r.name), true)));

  return { ratios: dedupedRatios, series, mcap, ev, debt };
}
...
function institutionalDCF(bundle, st, dcfIn, growth, ov = {}) {
  ...
  // terminal value, sensitivity grid) matches the Excel DCF Engine's WACC
  // scalar exactly. The previous code used `dcfIn.wacc` directly — which
  // `dcfDefaults` sets to (rf + β × ERP), i.e. cost-of-EQUITY, with the
  // explicit comment "WACC proxied by cost of equity". The Excel always
  // computed proper WACC = We × Ke + Wd × Kd×(1−T), so the in-app implied
  // value diverged from the workbook by ~Wd × (Ke − Kd×(1−T)) — small in
  // absolute terms (~0.05–0.10 pp) but compounded over 10y mid-year
  // discounting and through the terminal value, drove a ~5% gap in the
  // implied per-share value. We now compute the same proper WACC analytics-
  ...
  // the caller's object (which is shared across cache hits).
  dcfIn = { ...dcfIn, wacc: +_waccProper.toFixed(4) };
  // ── Opt-in direct discount-rate override (reverse-DCF / tornado engine) ──
  // The proper-WACC recompute above intentionally wins over dcfIn.wacc for
  // normal calls. Internal solvers (implied-WACC bisection, one-way WACC
  // sensitivity) need to pin the discount rate exactly, so they pass
  // ov.waccOverridePct. Regular API callers never set this → zero behavior
  // change on every existing path.
  if (ov.waccOverridePct != null && isFinite(+ov.waccOverridePct)) {
    dcfIn = { ...dcfIn, wacc: +(+ov.waccOverridePct).toFixed(4) };
  }
  ...
  function schedule(g1, marginDelta) {
    ...
    for (let y = 1; y <= horizon; y++) {
      ...
      // ── BUG FIX (v42): Use mid-year discount convention (t = y − 0.5),
      // matching the Excel DCF Engine. Cash flows accrue roughly uniformly
      // through the fiscal year, so the unbiased estimator places the
      // discount point at the midpoint of each period; end-of-year
      // discounting (t = y) was producing a ~(1+WACC)^0.5 − 1 ≈ 5–6% lower
      // implied price in-app than in the Excel export. Now both use the
      // same convention so on-screen Intrinsic Value and the Excel
      // workbook's implied per-share value reconcile to the cent.
      const t      = y - 0.5;
  ...
  // WACC build — reuse the values computed up front so `r` (the discount
  // rate inside schedule), `waccBuild.wacc` (returned to clients & shown
  // on the cover/WACC sheets), and the sensitivity grid all reference the
  // SAME proper WACC. mcap can now be re-derived using base.perShare as
  // the price fallback (only relevant when currentPrice is missing), but
  // we keep dcfIn.wacc fixed to avoid a circular dependency between
  // mcap → wd → wacc → schedule.
  const rf = _rfW, beta = _betaW, erp = _erpW;
  const costEquity = _costEquity;
  const costDebt = _costDebtAT;
...
/* ════════════════════════════════════════════════════════════════════════════
   ...
   Holds every assumption in the user's current set constant and solves, by
   bisection, for (a) the uniform first-stage revenue growth and (b) the
   discount rate (WACC) at which the model's intrinsic value equals the
   current market price. Both solvers re-run the SAME institutionalDCF engine
   used everywhere else — no parallel math, so the implied numbers reconcile
   exactly with the on-screen model and the Excel export.
   Deterministic, pure; the LLM never touches these figures.
   ════════════════════════════════════════════════════════════════════════════ */
function reverseDCF(bundle, st, dcfIn, growth, ov = {}, currentPrice) {
  const cp = +currentPrice;
  if (!isFinite(cp) || cp <= 0) return null;

  // Yearwise revenue-growth overrides would decouple the model from the
  // growthY1_5 scalar we bisect on, so the implied-growth solve strips ONLY
  // the growth array (margins/capex/tax/wc yearwise edits are preserved —
  // "given YOUR cost structure, what growth is priced in?").
  const ywNoGrowth = ov.yearwise ? { ...ov.yearwise, growth: undefined } : undefined;
  const ovG = { ...ov, yearwise: ywNoGrowth };

  const runG = (g) => {
    const r = institutionalDCF(bundle, st, { ...dcfIn, growthY1_5: g }, growth, ovG);
    return r && !r.error && r.base && isFinite(r.base.perShare) ? r.base.perShare : null;
  };
  const runW = (w) => {
    const r = institutionalDCF(bundle, st, dcfIn, growth, { ...ov, waccOverridePct: w });
    return r && !r.error && r.base && isFinite(r.base.perShare) ? r.base.perShare : null;
  };

  // Generic bisection. fPx must be monotonic in x over [lo, hi].
  //   increasing=true  → per-share rises with x (growth)
  //   increasing=false → per-share falls with x (WACC)
  const solve = (fPx, lo, hi, increasing) => {
    let pLo = fPx(lo), pHi = fPx(hi), iters = 0;
    if (pLo == null || pHi == null) return { value: null, bounded: false, iters };
    const below = increasing ? pLo : pHi;   // price at the "cheap" end
    const above = increasing ? pHi : pLo;   // price at the "rich" end
    if (below >= cp) return { value: increasing ? lo : hi, bounded: false, side: "below", iters };
    if (above <= cp) return { value: increasing ? hi : lo, bounded: false, side: "above", iters };
    for (iters = 0; iters < 60; iters++) {
      const mid = (lo + hi) / 2;
      const pm = fPx(mid);
      if (pm == null) return { value: null, bounded: false, iters };
      if (Math.abs(pm - cp) < cp * 1e-5) return { value: mid, bounded: true, iters };
      const midHigh = pm > cp;
      if (increasing) { if (midHigh) hi = mid; else lo = mid; }
      else            { if (midHigh) lo = mid; else hi = mid; }
    }
    return { value: (lo + hi) / 2, bounded: true, iters };
  };

  // Implied growth: search a wide but sane band.
  const gSol = solve(runG, -15, 60, true);
  // Implied WACC: must stay above terminal growth for the perpetuity to exist.
  const wLo = Math.max((dcfIn.terminalG ?? 3) + 0.35, 1);
  const wSol = solve(runW, wLo, 30, false);

  // Base model at the current assumption set (for the "vs your assumption" delta).
  const baseRun = institutionalDCF(bundle, st, dcfIn, growth, ov);
  const basePx = baseRun && !baseRun.error && baseRun.base ? baseRun.base.perShare : null;

  const round2 = (v) => (v == null ? null : Math.round(v * 100) / 100);
  return {
    currentPrice: cp,
    basePerShare: round2(basePx),
    impliedGrowth: round2(gSol.value),
    impliedGrowthBounded: gSol.bounded,           // false ⇒ answer lies outside [-15, 60]
    impliedGrowthSide: gSol.side || null,          // "below"/"above" when unbounded
    assumedGrowth: round2(dcfIn.growthY1_5),
    impliedWacc: round2(wSol.value),
    impliedWaccBounded: wSol.bounded,
    impliedWaccSide: wSol.side || null,
    assumedWacc: round2(dcfIn.wacc),
  };
}

/* ════════════════════════════════════════════════════════════════════════════
   TORNADO SENSITIVITY — one-way per-assumption impact on value per share.
   Each driver is flexed down/up by an institutional-standard step while all
   others hold at the user's current values; yearwise per-year edits are
   shifted in parallel so the flex respects user-entered schedules. Bars are
   sorted by total swing — the classic tornado ordering.
   ════════════════════════════════════════════════════════════════════════════ */
function tornadoAnalysis(bundle, st, dcfIn, growth, ov = {}, idcf) {
  if (!idcf || idcf.error || !idcf.base || !isFinite(idcf.base.perShare)) return null;
  const basePx = idcf.base.perShare;
  const a = idcf.assumptions || {};

  // Shift a yearwise array's defined entries by delta (percent points).
  const shiftYw = (yw, key, d) => {
    if (!yw || !Array.isArray(yw[key])) return yw;
    return { ...yw, [key]: yw[key].map((v) => (v != null && isFinite(+v) ? +v + d : v)) };
  };

  const run = (dcfDelta = {}, ovDelta = {}, ywKey = null, ywShift = 0) => {
    const yw = ywKey ? shiftYw(ov.yearwise, ywKey, ywShift) : ov.yearwise;
    const r = institutionalDCF(bundle, st, { ...dcfIn, ...dcfDelta }, growth, { ...ov, ...ovDelta, yearwise: yw });
    return r && !r.error && r.base && isFinite(r.base.perShare) ? r.base.perShare : null;
  };

  // [key, label, step(pp), low(), high(), baseDisplay]
  const drivers = [
    ["growth", "Revenue growth (Y1–5)", 3,
      () => run({ growthY1_5: dcfIn.growthY1_5 - 3 }, {}, "growth", -3),
      () => run({ growthY1_5: dcfIn.growthY1_5 + 3 }, {}, "growth", +3),
      a.growthY1_5],
    ["margin", "EBITDA margin", 2,
      () => run({}, { ebitdaMargin: (a.ebitdaMargin - 2) / 100 }, "ebitdaMargin", -2),
      () => run({}, { ebitdaMargin: (a.ebitdaMargin + 2) / 100 }, "ebitdaMargin", +2),
      a.ebitdaMargin],
    ["wacc", "WACC", 1,
      () => run({}, { waccOverridePct: a.wacc + 1 }),   // +1pp WACC → LOW value
      () => run({}, { waccOverridePct: a.wacc - 1 }),   // −1pp WACC → HIGH value
      a.wacc],
    ["terminalG", "Terminal growth", 0.5,
      () => run({ terminalG: dcfIn.terminalG - 0.5 }),
      () => run({ terminalG: dcfIn.terminalG + 0.5 }),
      a.terminalG],
    ["capex", "Capex % of revenue", 1,
      () => run({}, { capexPctRev: (a.capexPctRev + 1) / 100 }, "capexPctRev", +1),  // more capex → LOW
      () => run({}, { capexPctRev: Math.max(0, a.capexPctRev - 1) / 100 }, "capexPctRev", -1),
      a.capexPctRev],
    ["tax", "Effective tax rate", 2,
      () => run({}, { taxRate: Math.min(0.6, (a.taxRate + 2) / 100) }, "taxRate", +2), // higher tax → LOW
      () => run({}, { taxRate: Math.max(0, (a.taxRate - 2) / 100) }, "taxRate", -2),
      a.taxRate],
    ["wc", "Incremental WC % of Δrevenue", 1,
      () => run({}, { wcPctRev: (a.wcPctRev + 1) / 100 }, "wcPctRev", +1),            // more WC drag → LOW
      () => run({}, { wcPctRev: Math.max(0, a.wcPctRev - 1) / 100 }, "wcPctRev", -1),
      a.wcPctRev],
  ];

  const round2 = (v) => (v == null ? null : Math.round(v * 100) / 100);
  const bars = [];
  for (const [key, label, step, lowFn, highFn, baseVal] of drivers) {
    let lowPx = null, highPx = null;
    try { lowPx = lowFn(); highPx = highFn(); } catch { /* driver skipped */ }
    if (lowPx == null && highPx == null) continue;
    // Guard against non-monotonic edge cases: enforce low ≤ high for display.
    if (lowPx != null && highPx != null && lowPx > highPx) { const t = lowPx; lowPx = highPx; highPx = t; }
    bars.push({
      key, label, step, base: round2(baseVal),
      lowPx: round2(lowPx), highPx: round2(highPx),
      swing: round2((highPx ?? basePx) - (lowPx ?? basePx)),
      swingPct: basePx ? round2((((highPx ?? basePx) - (lowPx ?? basePx)) / basePx) * 100) : null,
    });
  }
  bars.sort((x, y) => Math.abs(y.swing ?? 0) - Math.abs(x.swing ?? 0));
  return { basePerShare: round2(basePx), bars };
}

/* ════════════════════════════════════════════════════════════════════════════
   DUPONT DECOMPOSITION — 3-stage and 5-stage, per fiscal year, with a YoY
   ROE-change attribution for the latest year. All inputs come straight from
   the normalized statements; every figure is computed, never estimated.
     3-stage: ROE = Net margin × Asset turnover × Equity multiplier
     5-stage: ROE = Tax burden × Interest burden × EBIT margin
                    × Asset turnover × Equity multiplier
   ════════════════════════════════════════════════════════════════════════════ */
function computeDuPont(st) {
  const inc = st.income || [], bal = st.balance || [];
  const balBy = Object.fromEntries(bal.map((r) => [r.year, r]));
  const rows = [];
  for (const li of inc) {
    const lb = balBy[li.year] || {};
    const rev = n(li.revenue), ni = n(li.netIncome), pretax = n(li.pretax);
    const ebit = n(li.ebit ?? li.opIncome), assets = n(lb.assets), equity = n(lb.equity);
    const netMargin = div(ni, rev);
    const turnover = div(rev, assets);
    const leverage = div(assets, equity);
    const taxBurden = div(ni, pretax);          // 1 − effective tax rate
    const intBurden = div(pretax, ebit);        // interest drag on EBIT
    const ebitMargin = div(ebit, rev);
    const roe3 = netMargin != null && turnover != null && leverage != null ? netMargin * turnover * leverage : null;
    rows.push({
      year: li.year,
      ...
      roe: roe3 != null ? roe3 * 100 : null,
    });
  }
  // YoY attribution on the 3-stage identity (log-decomposition — exact, the
  // component contributions sum to the total ROE log-change):
  //   Δln ROE = Δln margin + Δln turnover + Δln leverage
  let attribution = null;
  const cur = rows.at(-1), prev = rows.at(-2);
  const pos = (v) => v != null && isFinite(v) && v > 0;
  if (cur && prev && pos(cur.roe) && pos(prev.roe) &&
      pos(cur.netMargin) && pos(prev.netMargin) &&
      pos(cur.turnover) && pos(prev.turnover) &&
      pos(cur.leverage) && pos(prev.leverage)) {
    const dTot = Math.log(cur.roe / prev.roe);
    const dM = Math.log(cur.netMargin / prev.netMargin);
    const dT = Math.log(cur.turnover / prev.turnover);
    const dL = Math.log(cur.leverage / prev.leverage);
    const roeDeltaPp = cur.roe - prev.roe;                    // in percent points
    const share = (d) => (dTot !== 0 ? (d / dTot) * roeDeltaPp : 0);
    attribution = {
      fromYear: prev.year, toYear: cur.year,
      roeDeltaPp: Math.round(roeDeltaPp * 100) / 100,
      marginPp: Math.round(share(dM) * 100) / 100,
      turnoverPp: Math.round(share(dT) * 100) / 100,
      leveragePp: Math.round(share(dL) * 100) / 100,
    };
  }
  return rows.length ? { rows, attribution } : null;
}

/* ════════════════════════════════════════════════════════════════════════════
   XIRR — money-weighted annualized return over dated cashflows (bisection).
   flows: [{ date: ms | ISO string, amount }] — outflows (buys) negative,
   inflows (sells + terminal value) positive. Returns percent, or null when
   the flows can't define a rate (single-signed, <2 flows, no bracket).
   Same convention as Excel's XIRR (ACT/365.25 year fractions).
   ════════════════════════════════════════════════════════════════════════════ */
function xirr(flows) {
  const fs = (flows || [])
    .map((f) => ({ t: new Date(f.date).getTime(), a: +f.amount }))
    .filter((f) => Number.isFinite(f.t) && Number.isFinite(f.a) && f.a !== 0);
  if (fs.length < 2) return null;
  if (!fs.some((f) => f.a > 0) || !fs.some((f) => f.a < 0)) return null;
  const t0 = Math.min(...fs.map((f) => f.t));
  const yr = fs.map((f) => ({ y: (f.t - t0) / (365.25 * 24 * 3600 * 1000), a: f.a }));
  const npv = (r) => yr.reduce((s, f) => s + f.a / Math.pow(1 + r, f.y), 0);
  let lo = -0.9999, hi = 10;
  ...
  for (let i = 0; i < 200; i++) {
    ...
    if (Math.abs(fm) < 1e-8) return Math.round(mid * 10000) / 100;
...
/* ════════════════════════════════════════════════════════════════════════════
   HISTORICAL MULTIPLE BANDS — where does today's valuation sit against the
   stock's own history? Trailing P/E and P/B computed monthly over the price
   history, each month's denominator being the LAST-REPORTED fiscal year's
   EPS / book value per share (the information an investor actually had).
   Returns per-multiple { series, current, min, p25, med, p75, max, pctile }.
   Deterministic; approximation (annual denominators, calendar-year mapping)
   is stated in the note so the UI can disclose it.
   ════════════════════════════════════════════════════════════════════════════ */
function computeMultipleBands(monthly, st, sharesOut) {
  const pts = (monthly || []).filter((p) => p && p.c != null && Number.isFinite(p.t));
  if (pts.length < 12 || !sharesOut || !Number.isFinite(sharesOut) || sharesOut <= 0) return null;
  const perShareBy = (rows, key) => {
    const m = {};
    (rows || []).forEach((r) => { if (r[key] != null && Number.isFinite(r[key])) m[r.year] = r[key] / sharesOut; });
    return m;
  };
  const epsBy = perShareBy(st.income, "netIncome");
  const bvBy = perShareBy(st.balance, "equity");
  const denomFor = (map, calYear) => {
    let v = null;
    for (const y of Object.keys(map).map(Number).sort((a, b) => a - b)) if (y <= calYear) v = map[y];
    return v;
  };
  const build = (map) => {
    const series = pts.map((p) => {
      const d = denomFor(map, new Date(p.t).getFullYear());
      return { t: p.t, v: d != null && d > 0 ? p.c / d : null };
    });
    const clean = series.map((x) => x.v).filter((x) => x != null && Number.isFinite(x) && x > 0);
    if (clean.length < 8) return null;
    const sorted = [...clean].sort((a, b) => a - b);
    const q = (p) => sorted[Math.min(sorted.length - 1, Math.round(p * (sorted.length - 1)))];
    const current = [...series].reverse().find((x) => x.v != null)?.v ?? null;
    const pctile = current != null ? Math.round((sorted.filter((x) => x <= current).length / sorted.length) * 100) : null;
    const r2 = (v) => (v == null ? null : Math.round(v * 100) / 100);
    return {
      series: series.map((x) => ({ t: x.t, v: r2(x.v) })),
      current: r2(current), pctile,
      min: r2(sorted[0]), p25: r2(q(0.25)), med: r2(q(0.5)), p75: r2(q(0.75)), max: r2(sorted.at(-1)),
    };
  };
  const pe = build(epsBy);
  const pb = build(bvBy);
  if (!pe && !pb) return null;
  return {
    pe, pb,
    note: "Trailing multiples: each month's price over the last-reported fiscal year's EPS / book value per share (calendar-year mapping, current share count). Directional band, not a tick-accurate TTM series.",
  };
}

module.exports = { normStatements, computeRatios, computeGrowth, varianceAnalysis, dcfDefaults, runDCF, institutionalDCF, forensicScores, riskAssessment, multiValuation, monteCarlo, correlationMatrix, annVol, maxDrawdown, momentum, ruleNarrative, returns, stdev, assumptionEvidence, enrichDiagnostics, reverseDCF, tornadoAnalysis, computeDuPont, xirr, computeMultipleBands };
