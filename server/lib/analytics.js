...
function institutionalDCF(bundle, st, dcfIn, growth, ov = {}) {
  ...
  function schedule(g1, marginDelta) {
    ...
    for (let y = 1; y <= 5; y++) {
      const g = (g1 - dcfIn.fade * Math.max(0, y - 2)) / 100;
      rev = rev * (1 + Math.max(g, gT));
      const margin = Math.min(0.6, ebitdaMargin + (marginDelta / 100) * (y / 5));
      const ebitda = rev * margin;
      const dep = rev * depPctRev;
      const ebit = ebitda - dep;
      const nopat = ebit * (1 - taxRate);
      const capex = rev * capexPctRev;
      const dWC = (rev - prevRev) * wcPctRev;
      const fcff = nopat + dep - capex - dWC;
      const df = 1 / Math.pow(1 + r, y);
      const pv = fcff * df;
      pvSum += pv;
      rows.push({ year: baseYear + y, rev, growth: g * 100, ebitda, margin: margin * 100, dep, ebit, tax: ebit * taxRate, nopat, capex, dWC, fcff, df, pv });
      prevRev = rev;
    }
    const lastFcff = rows[rows.length - 1].fcff;
    const tv = (lastFcff * (1 + gT)) / (r - gT);
    const tvPv = tv * (1 / Math.pow(1 + r, 5));
    const ev = pvSum + tvPv;
  ...
  for (const dw of waccSteps) {
    ...
    for (const dg of gSteps) {
      ...
      const lastFcff = base.rows[base.rows.length - 1].fcff;
      const tv = (lastFcff * (1 + gg)) / (rr - gg);
      let pvSum = 0; base.rows.forEach((rw, idx) => pvSum += rw.fcff / Math.pow(1 + rr, idx + 1));
      const ev = pvSum + tv / Math.pow(1 + rr, 5);
  ...
  return {
    ...
    assumptions: { ebitdaMargin: ebitdaMargin * 100, depPctRev: depPctRev * 100, capexPctRev: capexPctRev * 100, taxRate: taxRate * 100, wcPctRev: wcPctRev * 100, growthY1_5: dcfIn.growthY1_5, fade: dcfIn.fade, terminalG: dcfIn.terminalG, wacc: dcfIn.wacc },
...
module.exports = { normStatements, computeRatios, computeGrowth, varianceAnalysis, dcfDefaults, runDCF, institutionalDCF, forensicScores, riskAssessment, multiValuation, monteCarlo, correlationMatrix, annVol, maxDrawdown, momentum, ruleNarrative, returns, stdev };
