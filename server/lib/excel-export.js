...
// Style kit shared with the consolidated research workbook (research-workbook.js)
// so every MERIDIAN export carries the identical institutional look.
module.exports.kit = { C, NF, STYLE, fontBase, fillSolid, border, allBorders, col, addr, fy, setCell, bannerRow, pageHeader };
...
function addCoverSheet(wb, p) {
  ...
  let r = 11;
  // ── BUG FIX (v42): the meta block must reflect the user's actual export
  // settings — including the sector, and the terminal-value method chosen
  // in the application. The previous block:
  //   • omitted Sector entirely (it appeared only inline in the subtitle);
  //   • showed Terminal Growth unconditionally even when the user selected
  //     Exit EV/EBITDA (so the Excel implied perpetual-growth reasoning the
  //     user never asked for).
  // Now: Sector is a first-class row, and the terminal section swaps between
  // "Terminal growth" (perpetual mode) and "Exit EV/EBITDA multiple" (exit
  // multiple mode) based on the assumption set.
  const isExitMult = p.assumptions?.terminalMethod === "exitMultiple";
  const horizonYrs = p.assumptions?.forecastHorizon || p.uiState?.forecastHorizon || 5;
  const meta = [
    ["Sector",            p.meta.sector || "n/a"],
    ["Currency",          p.ccy],
    ["Display unit",      p.unit],
    ...
    ["Forecast horizon",  `${horizonYrs} years`],
    ["Terminal method",   isExitMult ? "Exit EV/EBITDA" : "Perpetual Growth"],
  ];
  if (isExitMult) {
    meta.push(["Exit EV/EBITDA multiple",
      p.assumptions?.exitMultiple != null ? `${(+p.assumptions.exitMultiple).toFixed(2)}×` : "n/a"]);
  } else {
    meta.push(["Terminal growth",
      p.assumptions?.terminalG != null ? `${(+p.assumptions.terminalG).toFixed(2)}%` : "n/a"]);
  }
  meta.forEach((row) => {
    ...
    for (let y = 1; y <= nF; y++) {
      ...
      if (ov != null) {
        ...
        if (y === 1) {
          ...
          // ── BUG FIX (v42): high-growth period — Y2 default equals Y1
          // (no fade yet). Analytics treats Y1/Y2 as the institutional
          // "high growth" plateau and only fades from Y3 onward. The
          // Excel previously faded starting Y2 (drop of 1× Fade between
          // Y1 and Y2), so the year-by-year revenue path — and therefore
          // every downstream FCFF, EV, equity, and implied per-share —
          // differed from the in-app model by ~5% at typical fade rates.
          // Y2 now mirrors Y1; the cascade still applies from Y3 onward
          // via the prevColRef chain below.
          formula = `${prevColRef}`;
        } else {
          if (d.fade) {
...
function addSensitivity(wb, p) {
  ...
  const horizon = p.assumptions?.forecastHorizon || p.uiState?.forecastHorizon || 5;
  const nF = horizon;
  // ── BUG FIX (v42): Sensitivity grid axis must mirror the user's TV
  // method. Previously, the y-axis was always Terminal-g — meaningless
  // when the model is built on Exit EV/EBITDA. We now switch the y-axis
  // (and the per-cell TV formula) between Terminal-g (perpetual mode) and
  // Exit Multiple (exit mode). The x-axis (WACC) stays put in both modes.
  const isExitMult = p.assumptions?.terminalMethod === "exitMultiple";
  ws.columns = [
    { width: 3 }, { width: 24 },
  ...
  pageHeader(ws, "Sensitivity Analysis",
    isExitMult
      ? `Per-share value (${p.ccySym}) — WACC × Exit EV/EBITDA multiple grid · all 25 cells are live formulas`
      : `Per-share value (${p.ccySym}) — WACC × Terminal growth grid · all 25 cells are live formulas`,
    9);
  ...
  // ── 5×5 grid (axis depends on TV method) ───────────────────────────────
  bannerRow(ws, r, 2, 9,
    isExitMult
      ? "3.  PER-SHARE VALUE GRID  (rows = Exit EV/EBITDA multiple, columns = WACC)"
      : "3.  PER-SHARE VALUE GRID  (rows = Terminal growth, columns = WACC)");
  r++;

  // Axis values: WACC: base ± 1pp/2pp.
  // Y-axis: Terminal-g ± 0.5pp/1pp in perpetual mode, or
  //         Exit Multiple ± 1.0×/2.0× in exit-multiple mode.
  const waccSteps = [-2.0, -1.0, 0.0, 1.0, 2.0];
  const gSteps    = [-1.0, -0.5, 0.0, 0.5, 1.0];
  const mSteps    = [-2.0, -1.0, 0.0, 1.0, 2.0];

  // Header row (top-left corner cell, then 5 WACC values)
  setCell(ws, r, 2,
    isExitMult ? "Exit M  ↓  /  WACC →" : "Term. g  ↓  /  WACC →",
    STYLE.colHeader);
  for (let i = 0; i < 5; i++) {
    const c = 3 + i;
  ...
  for (let gi = 0; gi < 5; gi++) {
    ...
    const mDelta = mSteps[gi];
    // Row label
    if (isExitMult) {
      setCell(ws, r, 2, { formula: `ExitMult + ${mDelta}` }, STYLE.colHeader, NF.mult);
    } else {
    ...
    for (let wi = 0; wi < 5; wi++) {
      ...
      const ebitdaLast = `${col(2 + nF)}${ebitdaRow}`;
      let formula;
      if (isExitMult) {
        // TV(exit) = EBITDA(yN) × (ExitMult + mDelta); PV TV = TV / (1+wacc)^(N-0.5)
        const mExpr = `(ExitMult + ${mDelta})`;
        formula =
          `IF(${wExpr} > 0.5,` +
          ` ((SUMPRODUCT(Sens_FCFF, 1/((1+${wExpr}/100)^Sens_Period))` +
          ` + (${ebitdaLast} * ${mExpr}) / ((1+${wExpr}/100)^(${nF}-0.5))` +
          ` - NetDebtUsed) * ${epsFactor} / SharesMn),` +
          ` "n/m")`;
      } else {
        // TV(gordon) = FCFF(yN)*(1+g)/(wacc - g); PV TV = TV/(1+wacc)^(N-0.5)
  ...
  const notes = isExitMult
    ? [
        "• Each cell is a live SUMPRODUCT-based formula — change any input on Assumptions and the entire grid recomputes.",
        "• If WACC ≤ 0.5%, the cell shows \"n/m\" (discount factor blows up).",
        "• WACC axis: base WACC ± 1pp and ± 2pp.   Exit-Multiple axis: ExitMult ± 1× and ± 2×.",
        "• Grid uses the EXIT-EV/EBITDA terminal value (TV = EBITDA(yN) × ExitMult), matching the model's TV method.",
      ]
    : [
        "• Each cell is a live SUMPRODUCT-based formula — change any input on Assumptions and the entire grid recomputes.",
        ...
        "• Grid uses Gordon perpetuity-growth TV, matching the model's TV method (TermMethod = 1).",
      ];
  notes.forEach((line) => {
...
function addScenarios(wb, p) {
  ...
  const horizon = p.assumptions?.forecastHorizon || p.uiState?.forecastHorizon || 5;
  const nF = horizon;
  // ── BUG FIX (v42): Column layout must accommodate the per-scenario year
  // block — which spans nF year columns starting at column C. The previous
  // layout fixed memo at col 9 (I), so a 7y/10y horizon's year columns
  // would overflow the memo column. Now the memo column floats to
  // max(9, 3+nF+1) so it always sits to the right of the last year column.
  // The right-hand 5-scenario header row still uses cols C..G (3..7); that
  // span is independent of the per-block year block and is unaffected.
  const memoCol = Math.max(9, 3 + nF + 1);
  // Columns: A margin, B label, C..(2+nF) = year columns, then a spacer,
  // then the memo column. Use 14-wide for year cols, 4-wide spacer, 28 for memo.
  const colsLayout = [{ width: 3 }, { width: 38 }];
  for (let i = 0; i < nF; i++) colsLayout.push({ width: 14 });
  // Ensure we have at least 5 year-width columns so the 5-scenario header
  // (cols C..G) renders cleanly even at nF=3.
  while (colsLayout.length < 7) colsLayout.push({ width: 14 });
  colsLayout.push({ width: 4 });   // spacer
  colsLayout.push({ width: 28 });  // memo
  ws.columns = colsLayout;
  ...
  setCell(ws, r, memoCol, "Notes", STYLE.colHeader);
  ws.getRow(r).height = 22;
  r++;

  // ── 1. Scenario delta inputs ──────────────────────────────────────────
  bannerRow(ws, r, 2, memoCol, "1.  SCENARIO DELTAS  (from ScenarioTable on Assumptions)"); r++;
  const deltaLabels = [
  ...
  deltaLabels.forEach(([lbl, colInTbl, src]) => {
    ...
    setCell(ws, r, memoCol, src, STYLE.note);
    deltaRows[lbl] = r;
    r++;
  });
  r++;
  ...
  // ── 2. WACC by scenario ───────────────────────────────────────────────
  bannerRow(ws, r, 2, memoCol, "2.  WACC BY SCENARIO"); r++;
  ...
  setCell(ws, r, memoCol, "Base WACC + scenario Δ", STYLE.note);
  r++;
  ...
  setCell(ws, r, memoCol, "Base g + scenario Δ", STYLE.note);
  r += 2;

  // ── 3. Year-by-year FCFF per scenario (compact summary) ───────────────
  bannerRow(ws, r, 2, memoCol, "3.  SUM OF EXPLICIT-PERIOD FCFF  (per scenario)"); r++;
  ...
  bannerRow(ws, r, 2, memoCol, "4.  HELPER FCFF SCHEDULE  (per scenario, per year)"); r++;
  ...
  // ── BUG FIX (v42): The previous code capped per-scenario year columns at
  // 5 via `Math.min(nF, 5)`. For 7y/10y horizons this truncated the per-
  // scenario PV(FCFF) sum to only the first 5 years, materially under-
  // valuing equity in every scenario. With the column widths above now
  // sized for nF year columns, we render the full horizon.
  const colsForYears = nF;

  const scenBlockRows = []; // { scenIdx, perShareRow, evRow, equityRow }

  for (let s = 0; s < 5; s++) {
    // Banner for this scenario
    bannerRow(ws, r, 2, memoCol, `${String.fromCharCode(65 + s)}.  ${scenNames[s]} scenario  (scenario id = ${s + 1})`); r++;
    setCell(ws, r, 2, "Line item", STYLE.colHeader);
    for (let y = 1; y <= colsForYears; y++) {
      setCell(ws, r, 2 + y, { formula: `"FY"&RIGHT(BaseYear+${y},2)&"E"` }, STYLE.colHeader);
    }
    setCell(ws, r, memoCol, "Memo", STYLE.colHeader);
    r++;
    ...
    // ── BUG FIX (v42): Terminal value must honor the user's TermMethod
    // selection (Gordon perpetual vs Exit EV/EBITDA). Previously this block
    // computed only Gordon TV regardless of the application's TV-method
    // toggle, so any scenario in an Exit-Multiple model used the wrong TV
    // (and the scenario per-share values diverged from the DCF Engine sheet
    // that DOES branch via IF(TermMethod=1, …) ). We now compute both
    // candidates and pick via IF(TermMethod=1, …) — exactly mirroring the
    // DCF Engine's behavior.
    setCell(ws, r, 2, "Terminal FCFF = FCFF(yN) × (1+g)  [Gordon only]", STYLE.rowLabel);
    const tfR = r;
    setCell(ws, r, 3,
    ...
    const tvExitR = r;
    setCell(ws, r, 3,
      { formula: `${col(2 + colsForYears)}${ebR} * ExitMult` },
      STYLE.formula, NF.money);
    r++;
    setCell(ws, r, 2, "TV chosen  (= Gordon if TermMethod=1 else Exit-multiple)", STYLE.subtotalLabel);
    const tvR = r;
    setCell(ws, r, 3,
      { formula: `IF(TermMethod=1, ${col(3)}${tvGordonR}, ${col(3)}${tvExitR})` },
      STYLE.subtotal, NF.money);
    r++;
  ...
  bannerRow(ws, r, 2, memoCol, "5.  CONSOLIDATED PER-SHARE OUTCOMES"); r++;
  setCell(ws, r, 2, "Scenario", STYLE.colHeader);
  ...
  bannerRow(ws, r, 2, memoCol, "6.  PROBABILITY-WEIGHTED EXPECTED PRICE"); r++;
  ...
  setCell(ws, r, memoCol, "Edit blue cells; should sum to 100%.", STYLE.note);
  r++;
  ...
  setCell(ws, r, 3,
    ...
    STYLE.total, NF.money2);
  setCell(ws, r, memoCol, "Weighted by user-supplied probabilities.", STYLE.note);
  r++;

  ws.pageSetup.printArea = `A1:${col(memoCol)}${r}`;
}
...
function addDashboard(wb, p) {
  ...
  // KPI cells (4 across × 2 rows). The last KPI tile switches based on
  // the TV method — Terminal Growth (perpetual) or Exit EV/EBITDA Multiple
  // (exit-mult) — so the dashboard reflects the assumption set the user
  // actually selected.
  const dashIsExitMult = p.assumptions?.terminalMethod === "exitMultiple";
  const kpis = [
    ...
    dashIsExitMult
      ? { lbl: "Exit EV/EBITDA multiple", val: { formula: `ExitMult` }, fmt: NF.mult, color: C.black, sub: "× terminal-year EBITDA" }
  ...
  setCell(ws, r, 2 + nF + 1, `${nF}y CAGR`, STYLE.colHeader);
  r++;
